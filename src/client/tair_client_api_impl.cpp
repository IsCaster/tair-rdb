...
FAIL:
    ...
    int tair_client_impl::get(int area, const data_entry &key, data_entry *&data )
    {
        ...
        if(ret != TAIR_RETURN_SUCCESS) goto FAIL;
...
FAIL:
    ...
    // zrange
    // zrange将发送一个request_zrange类型的请求
    // 这个请求将在tair_server.cpp的handlePacketQueue函数（245行）的第541行被处理
    // 具体处理过程是由一个request_processor类型的实例调用process方法处理
    // process函数有多个不同的签名，分别处理不同的请求
    // 处理request_zrange的process函数在request_processor.cpp的第649行
    // 处理过程是通过tair_mgr的storage_mgr调用zrange实现的
    // storage_mgr的zrange是一个虚方法
    // 有很多不同类型的存储方案，到底使用哪个存储引擎是由配置文件决定的，此处使用的是rdb
    // 因此调用的是/storage/rdb/rdb_manager.h里面的zrange方法
