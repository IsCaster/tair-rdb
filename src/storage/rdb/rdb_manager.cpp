                                      ...
                                      std::vector<data_entry *> &values, std::vector<double> &scores, int limit, int withscore, bool reverse)
{
    ...
    log_debug("rdb_manager::genericZrangebyscore key=%s,size=%d,hex=%s withscore=%d", key.get_data() + 2, key.get_size(), hexStr(key.get_data(), key.get_size()).c_str(), withscore);
    value_item *items = NULL;
    int items_len = 0;
    double *scores_items = NULL;
    int scores_items_len = 0;
